      ...
      onMounted(async () => {
        try {
          ...
          movimientos.forEach(movimiento => {
            if (movimientosPorProducto[movimiento.product_id]) {
              movimientosPorProducto[movimiento.product_id]++;
            } else {
              movimientosPorProducto[movimiento.product_id] = 1;
            }
          });
          ...
          productos.forEach(producto => {
            ...
            movimientosData.push(movimientosPorProducto[producto.id] || 0);
          });
  
          // Generar colores aleatorios para los productos
          const coloresMovimientos = generarColoresAleatorios(productos.length);
